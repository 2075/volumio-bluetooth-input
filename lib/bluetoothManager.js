...
BluetoothManager.prototype.initialize = function (deviceName, discoverable) {
  ...
  return self._execBtctl('power on')
    ...
    .then(function () {
      return self._execBtctl('agent NoInputNoOutput');
    })
    .then(function () {
      return self._execBtctl('default-agent');
    })
    .then(function () {
...
BluetoothManager.prototype._startMonitor = function () {
  ...
  try {
    ...
    self.monitorProcess.on('close', function () {
      self.logger.info('BluetoothManager::monitor process closed');
...
BluetoothManager.prototype._parseMonitorLine = function (line) {
  ...
  if (cleanLine.match(/Authorize service/i)) {
    self.logger.info('BluetoothManager::auto-authorizing service');
    if (self.monitorProcess && self.monitorProcess.stdin) {
      self.monitorProcess.stdin.write('yes\n');
    }
  }
};
