...
// Returns devices that are formally paired (bonded) — used internally for reconnect
...
// Returns all devices BlueZ considers known and either Paired or Trusted.
// A TV or phone connecting via "Just Works" may only be Trusted (not Paired),
// so this is what the UI should use for the paired devices list.
BluetoothManager.prototype.getKnownDevices = function () {
  var self = this;
  return self._execCmd('bluetoothctl devices')
    .then(function (stdout) {
      var devices = parseDeviceList(stdout);
      if (devices.length === 0) return [];

      // Check each device in parallel; keep those that are Paired or Trusted,
      // and surface the isPaired flag so the UI can distinguish the two states.
      return Promise.all(devices.map(function (device) {
        return self._execCmd('bluetoothctl info ' + device.mac)
          .then(function (info) {
            var paired  = /Paired:\s*yes/i.test(info);
            var trusted = /Trusted:\s*yes/i.test(info);
            if (!paired && !trusted) return null;
            return { mac: device.mac, name: device.name, isPaired: paired };
          })
          .catch(function () { return null; });
      }))
      .then(function (results) {
        return results.filter(function (d) { return d !== null; });
      });
    })
    .catch(function () {
      return [];
...
// Sends a connect command through the persistent monitor process.
// Connection success/failure surfaces via the 'deviceConnected' event,
// so callers don't need to await a result here.
BluetoothManager.prototype.connectDevice = function (mac) {
  this._writeMonitor('connect ' + mac);
  return Promise.resolve();
};
...
BluetoothManager.prototype._parseMonitorLine = function (line) {
  ...
  if (connectMatch) {
    ...
    // Ensure every connecting device is trusted so it appears in getKnownDevices()
    // and can reconnect automatically next time
    self._writeMonitor('trust ' + mac);
