...
var LATENCY_PRESETS = {
  low:    { buffer: 60000,  period: 20000 },
  medium: { buffer: 200000, period: 50000 },
  high:   { buffer: 500000, period: 100000 }
};

function AudioService(logger) {
  this.logger = logger;
  this.outputDevice = 'plughw:0,0';
  this.latencyMode = 'low';
  this.activeStreams = {};
}

AudioService.prototype.start = function (outputDevice, latencyMode) {
  var self = this;
  self.outputDevice = outputDevice || 'plughw:0,0';
  self.latencyMode = latencyMode || 'low';
  return self._systemctl('start', 'bluealsa.service');
};

AudioService.prototype.setLatencyMode = function (mode) {
  var self = this;
  self.latencyMode = mode || 'low';
  ...
  self.logger.info('AudioService::setLatencyMode -> ' + mode + ', restarting ' + macs.length + ' stream(s)');
  var chain = Promise.resolve();
  macs.forEach(function (mac) {
    chain = chain.then(function () { return self.stopAplay(mac); })
                 .then(function () { return self.startAplay(mac); });
  });
  return chain;
};
...
AudioService.prototype._writeAplayOverride = function (mac, device) {
  var self = this;
  var preset = LATENCY_PRESETS[self.latencyMode] || LATENCY_PRESETS.low;
  var overrideDir = '/etc/systemd/system/bluealsa-aplay@' + mac + '.service.d';
  var overrideFile = overrideDir + '/output.conf';
  var execLine = '/usr/bin/bluealsa-aplay ' + mac +
    ' -D ' + device +
    ' --pcm-buffer-time=' + preset.buffer +
    ' --pcm-period-time=' + preset.period;
  var content = '[Service]\nExecStart=\nExecStart=' + execLine + '\n';

  self.logger.info('AudioService::aplay override: ' + execLine);
